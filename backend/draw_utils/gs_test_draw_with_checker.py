...
map_file_name = 'backend/maps/area4_global_inout_map.npy'
check_target = np.load(map_file_name)
check_map_img = np.zeros((1080, 1920, 3), np.uint8)
target_non_false_idx  = np.where(check_target==True)
check_map_img[target_non_false_idx] = (0,0,200)
...
while True:
    ...
    frame = cv2.addWeighted(frame, 0.7, check_map_img, 0.3, 0)
    ...
    if ret:
        ...
        # touch_rec_idx = np.where(touch_record!=(0,0,0))
        # touch_rec_idx = np.where((touch_record != [0, 0, 0]).all(axis=2))
        # touch_rec_idx = np.where((touch_record == (0, 0, 0)).all(axis=0)!=(0,0,0))
        ...
        # frame[touch_rec_idx] = np.ones_like(frame[touch_rec_idx])
        # result[blank_idx_complementary] = frame[blank_idx_complementary]



        # frame[touch_record.any(axis=2) != 0] = (0, 255, 0)
